...
const Faults = (props) => {
    ...
    const fetchFaults = async () => {
        const response = await axios.get(process.env.REACT_APP_HOST + `/api/faults/${props.currentHill.id}`);
        return response.data;
    }

    const sendFault = async () => {
        console.log(props.user.id, props.currentHill.id)
        await axios.post(process.env.REACT_APP_HOST + `/api/faults/addFault`, {
